...
class ReadEvent:
    ...
    def discount_return(self, code, usertype):
        ...
        if redis_event.hget(
            ...
            dis = float(redis_event.hget(f"discount:{code}:info", "Discount"))
        else:
    ...
    def buy_ticket(self, price):
        ...
        if price == self.total_price:
            redis_event.hmset(name=f"sold:{self.event_id}:info", mapping={
                "ID": redis_event.hget(f"event:{self.event_id}:info", "ID"),
                "Date": redis_event.hget(
                        f"event:{self.event_id}:info", "Date"),
                "Event Name": redis_event.hget(f"event:{self.event_id}:info",
                                               "Event Name"),
