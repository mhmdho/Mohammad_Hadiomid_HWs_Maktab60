...
class OtpView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # r = psycopg2(host='redis_db', port=6379, db=0)
        # r.set(f'otp:{phone}', otp)
        
        db = psycopg2.connect(database="test", user="postgres", host="postgres_db")
        cur = db.cursor()
        cur.execute(f"SELECT time FROM otp_table WHERE phone = '{phone}' ")       
        time = cur.fetchone()
        if time:
            if time[0] + timedelta(minutes=1) < datetime.now():
                cur.execute(f"DELETE FROM otp_table where phone='{phone}' ")
                cur.execute(f"INSERT INTO otp_table VALUES ({phone}, {otp})")
        else:
            cur.execute(f"INSERT INTO otp_table VALUES ({phone}, {otp})")
